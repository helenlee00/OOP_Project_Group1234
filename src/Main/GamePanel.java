...
public class GamePanel extends JPanel  implements Runnable{
    ...
    public Entity snowman[] = new Entity[20];
    public Entity bigsnowman[] = new Entity[1];
    ...
    public void update() {
        if(gameState == playState)
        {
            ...
            for(int i = 0; i < snowman.length; i++)
            {
                if(snowman[i] != null)
                {
                    if(snowman[i].alive == true && snowman[i].dying == false){
                    snowman[i].update();
                    }
                    if(snowman[i].alive == false){
                        snowman[i].checkDrop();
                        snowman[i] = null;
                        }
                }
            }
            for(int i = 0; i < bigsnowman.length; i++)
            {
                if(bigsnowman[i] != null)
                {
                    if(bigsnowman[i].alive == true && bigsnowman[i].dying == false){
                    bigsnowman[i].update();
                    }
